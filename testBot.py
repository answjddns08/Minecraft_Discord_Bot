...
SERVER_COMMAND = (
    "cd /home/redeyes/Documents/Minecraft/ && ./start.sh"  # 서버 실행 명령어
)

WORLDS_DIR = "/home/redeyes/Documents/MinecraftWorlds"  # 월드 파일들이 있는 디렉토리
TRASH_DIR = (
    "/home/redeyes/Documents/MinecraftWorldsTrash"  # 삭제된 월드들이 있는 디렉토리
)
    ...
    try:
        ...
        with MCRcon(RCON_HOST, RCON_PASSWORD, port=RCON_PORT) as mcr:
            ...
            if "There are" in response:
                players_list = (
                    response.split(": ")[1].strip() if ": " in response else ""
                )
                return players_list
            else:
                return ""
...
async def check_server_players(interaction: discord.Interaction):
    ...
    if response:
        await interaction.response.send_message(
            f"현재 서버 인원: {len(response.split(', '))}명\n플레이어 목록: {response}"
        )
    elif response == "":
...
async def start_server(interaction: discord.Interaction):
    ...
    if is_server_running():
        ...
        execute_command(
            f"tmux new-session -d -s {TMUX_SESSION_NAME} '{SERVER_COMMAND}'"
        )
        await interaction.response.send_message(
            f"{world} 월드를 시작합니다.\n실행하는데 시간이 좀 걸려요. ⏳"
        )
...
async def select_world(interaction: discord.Interaction, world_name: str):
    ...
    try:
        execute_command(
            f"mv ~/Documents/Minecraft/world* ~/Documents/MinecraftWorlds/{last_world}/ 2>/dev/null"
        )
    except Exception as e:
    ...
    try:
        execute_command(
            f"mv ~/Documents/MinecraftWorlds/{world_name}/world* ~/Documents/Minecraft/ 2>/dev/null"
        )
    except Exception as e:
        print(f"Error uploading world: {e}")
...
async def rename_world(
    interaction: discord.Interaction, current_name: str, new_name: str
):
    try:
        # lastWorld인지 확인
        last_world = dotenv.get_key(".env", "lastWorld").strip("'")
        ...
        if os.path.exists(os.path.join(WORLDS_DIR, new_name)):
            await interaction.response.send_message(
                f"'{new_name}' 이름의 월드가 이미 존재합니다. ❌"
            )
            return

        # 월드 이름 변경
        execute_command(
            f"mv ~/Documents/MinecraftWorlds/{current_name} ~/Documents/MinecraftWorlds/{new_name}"
        )
        ...
        await interaction.response.send_message(
            f"월드 이름을 변경했습니다:\n**{current_name}**  ➡️  **{new_name}**  ✅"
        )

    except Exception as e:
        print(f"Error renaming world: {e}")
        await interaction.response.send_message(
            "월드 이름을 변경하는 중 오류가 발생했습니다. ❌"
        )
...
async def create_world(interaction: discord.Interaction, world_name: str):
    try:
        ...
        await interaction.response.send_message(
            f"새로운 월드 **{world_name}**가 생성되었습니다. ✅"
        )

    except Exception as e:
        print(f"Error creating world: {e}")
        await interaction.response.send_message(
            "월드를 생성하는 중 오류가 발생했습니다. ❌"
        )
...
async def remove_world(interaction: discord.Interaction, world_name: str):
    ...
    try:
        ...
        # 월드를 쓰레기통으로 이동
        execute_command(
            f"mv ~/Documents/MinecraftWorlds/{world_name} ~/Documents/MinecraftWorldsTrash/"
        )
        ...
        await interaction.response.send_message(
            f"**{world_name}** 월드를 삭제했습니다. ✅"
        )

    except Exception as e:
        print(f"Error removing world: {e}")
        await interaction.response.send_message(
            "월드를 삭제하는 중 오류가 발생했습니다. ❌"
        )
...
async def restore_world(interaction: discord.Interaction, world_name: str):
    ...
    try:
        ...
        # 월드를 복구
        execute_command(
            f"mv ~/Documents/MinecraftWorldsTrash/{world_name} ~/Documents/MinecraftWorlds/"
        )
        ...
        await interaction.response.send_message(
            f"**{world_name}** 월드를 복구했습니다. ✅"
        )

    except Exception as e:
        print(f"Error restoring world: {e}")
        await interaction.response.send_message(
            "월드를 복구하는 중 오류가 발생했습니다. ❌"
        )
            ...
            if worlds:
                for world in worlds:
                    ...
                    if time_diff > lastTime * 24 * 60 * 60:
                        # 월드 삭제
                        execute_command(
                            f"rm -rf ~/Documents/MinecraftWorldsTrash/{world}"
                        )
...
async def squareMapMap(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        "squareMap 지도: https://notebook.o-r.kr/squaremap/"
    )
...
async def squareMapRender(interaction: discord.Interaction):
    ...
    try:
        ...
        with MCRcon(RCON_HOST, RCON_PASSWORD, port=RCON_PORT) as mcr:
            ...
            await interaction.response.send_message(
                "squareMap 렌더를 시작했습니다. ✅\n 전부 렌더가 되기까지 시간이 걸려요. ⏳"
            )
    except Exception as e:
        print(f"Error rendering squareMap: {e}")
        await interaction.response.send_message(
            "squareMap 렌더를 시작하는 중 오류가 발생했습니다. ❌"
        )
